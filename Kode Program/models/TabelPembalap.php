...
class TabelPembalap extends DB implements KontrakModel {
    ...
    public function getAllPembalap(): array {
        $query = "SELECT * FROM pembalap ORDER BY id DESC";
    ...
    // Method untuk menambah pembalap - DILENGKAPI
    ...
    // Method untuk update pembalap - DILENGKAPI
    ...
    // Method untuk hapus pembalap - DILENGKAPI
    public function deletePembalap($id): void {
        $query = "DELETE FROM pembalap WHERE id = :id";
        $this->executeQuery($query, ['id' => $id]);
    }

    // Method tambahan untuk kebutuhan Presenter - DIPERBAIKI
    public function insertPembalap($pembalap) {
        $this->addPembalap(
            $pembalap->getNama(),
            $pembalap->getTim(),
            $pembalap->getNegara(),
            $pembalap->getPoinMusim(),
            $pembalap->getJumlahMenang()
        );
        
        // Ambil ID terakhir dengan query SELECT (SOLUSI untuk private $conn)
